...
module.exports = {
    ...
    getConfig: function() {
        ...
        if (fs.existsSync(configPath)) {
            ...
            configObject.sourceLocation = makeArray(configObject.sourceLocation);
            configObject.destinationLocation = makeArray(configObject.destinationLocation);
            return configObject;
        } else {
            return {};
        }

        // function to convert old string filepaths into new array of filepaths format
        function makeArray (location) {
            if (!Array.isArray(location)) {
                if (typeof(location) !== 'string') {
                    throw new Error('config.sourceLocation and config.destinationLocation must be either a string or an array of strings');
                }
                var newArray = [];
