...
module.exports = {
    ...
    writeConfigToDev: function(configFile) {
        ...
        exec('scp ' + configPath + ' ' + configFile.userName + '@' + configFile.hostname + ':~/' + configDir, function(error) {
            if (error) throw new Error('We couldn\'t copy over the config to your devbox, please copy ~/.sicksync-config.json to your devbox in the same location');
    ...
    // Log only shows messages if the debug flag is true
    log: function() {
        if (!this.getConfig().debug) return false;
    ...
    wakeDevBox: function(host, fn) {
        ...
        sicksyncServer = fork(__dirname + '/../lib/server-start', null, {
            silent: true
        });
