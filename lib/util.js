...
module.exports = {
    ...
    getSicksyncDir: function() {
        return [this.getHome(), SICKSYNC_DIR].join('/');
    },

    // Return the path to the update json
    getUpdatePath: function() {
        return this.getSicksyncDir() + '/' + UPDATE_FILE;
    },

    // Returns the path the the config file
    getConfigPath: function() {
        return this.getSicksyncDir() + '/' + CONFIG_FILE;
    },
    ...
    writeConfig: function(configFile) {
        ...
        console.log(clc.green('Successfully wrote ' + configPath));
    },
    ...
    log: function() {
        var args = [clc.green('[' + hostname + ']')].concat([].slice.call(arguments));
