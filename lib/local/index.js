...
function start(projects, opts, config) {
    _.each(projects, function(project) {
        if (_.isEmpty(config.projects[project])) {
            console.log(text.PROJECT_NOT_FOUND, project);
            return false;
        }
...
function startProject (project, config) {
    ...
    var sourceLocation = untildify(projectConf.sourceLocation);
    ...
    fsHelper.on(fsEvents.CHANGE, function(data) {
        data.filepath = data.sourcepath.replace(sourceLocation, projectConf.destinationLocation);
...
function once(projects, opts, config) {
    _.each(projects, function(project) {
        ...
        log(text.SYNC_ON_ONCE);

        bigSync(projectConf, {
            dry: opts.dryRun,
            debug: config.debug
        }, _.partial(log, text.SYNC_ON_ONCE_DONE));
    });
}
