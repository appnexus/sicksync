...
function startProject (project, config) {
    ...
    wsClient.on('reconnecting', _.partial(localLog, text.SYNC_ON_RECONNECT));

    wsClient.on('disconnected', _.partial(localLog, text.SYNC_ON_DISCONNECT));

    wsClient.on('remote-error', function(err) {
