...
module.exports = {
    ...
    updateRemote: function() {
        ...
        ssh.stdout.on('data', function(data) {
            ...
            if (_.contains(message, 'sicksync@')) {
                console.log([
                    chalk.green(config.hostname + ' updated successfully!')
                ].join('\n'));
            ...
            if (_.contains(message, 'ERR!')) {
                console.log([
                    chalk.red(config.hostname + ' update failed: ' + message),
    ...
    updateLocal: function() {
        exec(constants.UPDATE_CMD, function (error, stdout, stderr) {
            if (!!error || _.contains(stderr, 'ERR!')) {
                return console.log([
                    chalk.red('There was a problem updating sicksync: ' + (error || stderr)),
                    'Please run `' + constants.UPDATE_CMD + '`'
                ].join('\n'));
            }

            console.log([
                chalk.green(hostname + ' updated successfully!')
            ].join('\n'));
        });
    },
    update: function() {
    ...
    notify: function() {
        if (updateInfo.version !== package.version) {
            return console.log([
                chalk.green('Sicksync update available!'),
                'Current version: ' + chalk.yellow(package.version),
                'Latest version: ' + chalk.green(updateInfo.version),
                'Run `' + chalk.green('sicksync update') + '` to update!'
            ].join('\n'));
        }
    },
    check: function() {
