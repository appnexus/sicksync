...
module.exports = function startRemote(opts) {
    if (!_.isNumber(opts.port)) return console.warn(clc.red('--port, -p, is required. See `sicksync remote -h`'));
    if (!_.isString(opts.secret)) return console.warn(clc.red('--secret, -s, is required. See `sicksync remote -h`'));
