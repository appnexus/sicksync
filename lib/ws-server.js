var WebSocketServer = require('ws').Server,
    EventEmitter = require('events').EventEmitter,
    util = require('./util'),
    crypt = require('./crypt'),
    config = util.getConfig(),
    isAuthorized = true, // TODO: change this to handle multiple authorizations, one per client
    _ws = null,
    events = null,
    wss = null,
    port = null
    clientCount = 0;

function WSServer(params) {
    port = params.port;
    setupServer();
    setupEventEmitter();

    return {
        on: events.on.bind(events)
        on: events.on.bind(events)
    };
}

function setupEventEmitter() {
    events = new EventEmitter();
}

function setupServer() {
    wss = new WebSocketServer({
        port: port
    });
    wss.on('connection', handleConnect);
    console.log('[' + config.hostname + '] Server is up and waiting for changes');
}

function handleConnect(ws) {
    clientCount++;
    console.log('[ws-server] client connection received');
    _ws = ws; // this is problematic for multiple connections
    ws.on('message', handleMessage);
    ws.on('close', connectionClosed);
}

function send(obj) {
    _ws.send(crypt.stringifyAndEncrypt(obj));
}

function handshake(message) {
       send({
            subject: 'handshake',
            isAllowed: isAuthorized
        });

}

function handleFileChange(message) {
    events.emit('file-change', message);
}

function handleMessage(message) {
    console.log('[ws-server] message received');
    var parsedMessage = crypt.decryptAndParse(message);

    if (parsedMessage.subject === 'handshake') {
        return handshake(parsedMessage);
    }

    if (isAuthorized &&  parsedMessage.subject === 'file') {
        return handleFileChange(parsedMessage);
    }

    // If we've received a message that isn't recognized, bail
    console.log('Server is closing');
    process.exit();
}

function connectionClosed() {
    console.log('[ws-server] client connection closed');
    if (clientCount > 0) {
        clientCount--;
    } else {
        console.error("ERROR: client disconnected but client count was " + clientCount);
    }
    events.emit('connection-closed', clientCount);
}

module.exports = WSServer;
